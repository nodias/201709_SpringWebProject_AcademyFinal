			...
			<table  class="table table-hover">
				...
				<c:choose>
					<c:when test="${listcount>0}">
						...
						<c:forEach var="actorList" items="${dataList}" varStatus="status">
							<tbody>
								<tr>
									<td><c:out value="${actorList.act_no}" /></td>
									<td><c:out value="${actorList.name}" /></td>
									<td><c:out value="${actorList.reg_date}" /><a href="./ActorDeleteProcess.jh?delNum=${actorList.act_no}"><button  class="btn btn-danger">삭제</button></a></td>
								</tr>
							</tbody>
						</c:forEach>
					</c:when>
